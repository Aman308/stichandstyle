...
const Collection = () => {
  ...
  const applyFilter = () =>{
    ...
    if (showSearch && search ) {
      productsCopy = productsCopy.filter(item => item.name.toLowerCase().includes(search.toString().toLowerCase()))
    }
 ...
 useEffect(()=>{
  ...
  return (
    ...
    <div className='min-w-60'>
      ...
      <p className='mb-3 text-sm font-medium '>Cattegories</p>
    ...
    <div className='flex-1'>
      <div className='flex justify-between text-2xl sm:text-2xl mb-4'>
        <Title text1={'All'} text2={'Collections'}/>
        {/* Product Sort */}
        <select onChange={(e)=>setSortType(e.target.value)} className='border-2 border-gray-300 text-sm px-3'>
          <option className='bg-gray-200' value="relavent">Sort by : Relavent</option>
          <option className='bg-gray-200' value="low-high">Sort by : Low-Hight</option>
          <option className='bg-gray-200' value="high-low">Sort by : High-Low</option>
        </select>
      </div>
